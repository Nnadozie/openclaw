...
<AccordionGroup>
  ...
  <Accordion title="Session control">
    ...
    - `sessions.status` (`operator.read`) returns bounded status facts without reading transcripts. See [Session status facts](/gateway/protocol/rpc-methods#session-status-facts).
